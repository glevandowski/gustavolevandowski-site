...
!!                                                                           !!
!!  This page is GENERATED. Any hand edit will be OVERWRITTEN on the next    !!
!!  Netlify / GitHub Actions build.                                          !!
!!                                                                           !!
!!  Edit the bilingual source instead:                                       !!
!!      src/index.html                                                       !!
!!                                                                           !!
!!  Then run:                                                                !!
!!      python scripts/build_i18n.py                                         !!
!!  (or just push — the pipeline rebuilds /en and /pt automatically.)        !!
!!                                                                           !!
!!  PARAR. NAO EDITE ESTE ARQUIVO MANUALMENTE.                               !!
!!  Pagina GERADA. Edite apenas: src/index.html                              !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
-->
""".lstrip()
...
def build_variant(lang: str) -> Path:
    ...
    if not html.lstrip().lower().startswith("<!doctype"):
        html = "<!DOCTYPE html>\n" + html
    # Keep the warning above <html> so it is the first thing anyone sees in the file.
    if html.lstrip().lower().startswith("<!doctype html>"):
        parts = html.split("\n", 1)
        html = parts[0] + "\n" + DO_NOT_EDIT_BANNER + (parts[1] if len(parts) > 1 else "")
    else:
        html = DO_NOT_EDIT_BANNER + html
